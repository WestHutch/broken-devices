...
from credentials import get_user_info, set_credentials, write_email_info
import argparse
...
def complete_ic(studentNum, user_credentials, context, assetTag, serialNumber, reason, todaysDate):
    ...
    page.fill('#username', user_credentials["ic_user"])
    page.fill('#password', user_credentials["ic_pw"])
...
def complete_destiny(context, user_credentials, serialNumber):
...
def complete_outlook(context, user_credentials, emailList, todaysDate, s1, reason):
    page2 = context.new_page()
    page2.goto('https://outlook.office.com/mail/')
    page2.get_by_placeholder("Email, phone, or Skype").click()
    page2.get_by_placeholder("Email, phone, or Skype").fill(user_credentials["email"])
    page2.get_by_role("button", name="Next").click()
    page2.get_by_placeholder("Password").click()
    page2.get_by_placeholder("Password").fill(user_credentials["email_pw"])
    ...
    page2.keyboard.type(user_credentials["boss_email"])
...
def complete_synetic(context, user_credentials, assetTag, serialNumber, reason):
    page3 = context.new_page()
    page3.goto('https://synetic.repairportal.com/en/orders')
    page3.fill('#username', user_credentials["synetic_user"])
    page3.fill('#password', user_credentials["synetic_pw"])
...
def complete_worthave(context, user_credentials, serialNumber, reason, incidentDesc):
    page3 = context.new_page()
    page3.goto('https://www.worthavegroup.com/customer/account/login/', timeout=180000)
    page3.get_by_role("textbox", name="Email").fill(user_credentials["worthave_user"])
    page3.get_by_role("textbox", name="Password").fill(user_credentials["worthave_pw"])
...
def main():
    ...
    parser.add_argument("--reset-email-info", action="store_true", dest="reset_email_info", help="Reset stored email information before running")
    args = parser.parse_args()

    if args.reset_credentials:
        set_credentials()
    
    if args.reset_email_info:
        write_email_info()

    user_credentials = get_user_info()
    emailList = user_credentials["emails"]
    ...
    with sync_playwright() as p:
        ...
        s1 = complete_ic(studentNum, user_credentials, context, assetTag, serialNumber, reason, todaysDate)
        #s1 = student(studentNum, "Man, Spider", "SM", "09", "Sp12345T28")
        complete_outlook(context, user_credentials, emailList, todaysDate, s1, reason)
        complete_destiny(context, user_credentials, serialNumber)
        if deviceType == "":
            if serialNumber[:4].upper() == "FVFH" or serialNumber[:4].upper() == "FVFF":
                complete_worthave(context, user_credentials, serialNumber, reason, incidentDesc)
            elif len(serialNumber) == 10: #m2
                complete_synetic(context, user_credentials, assetTag, serialNumber, reason)
        save_label(path, s1)
        print_label(path)
